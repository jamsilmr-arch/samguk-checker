// [시스템 분석] tactic_dogam.js - 전법 도감 테마 동기화 및 100% 무손실 엔진 (유비무환 추가 및 오염 데이터 삭제 완료)
console.log("[시스템 분석] tactic_dogam.js 고속 해시 맵 렌더러 기동 (계승 전법 79종 클린 렌더링)");

var cStr = s => s?.toString().trim().replace(/\s+/g, '') || "";

// 🚨 [데이터 교정] 장수 고유 전법(애자필보, 축호과간, 호치) 영구 삭제 및 '유비무환' 정규 추가 완료 (총 79종)
var tacticDogamData = [
    ...
    { id: 't_yubi', name: '유비무환', type: '지휘 (100%)', target: '아군 2팀', desc: '매 턴 시작 시 70% 확률로 아군 2명의 병력을 크게 회복시키고(치료율 150%, 모략의 영향을 받음), 전투 종료 시까지 아군이 받는 피해를 15% 감소시킵니다.' }, // 🚨 유비무환 추가
