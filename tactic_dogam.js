// [시스템 분석] tactic_dogam.js - 전법 도감 테마 동기화 및 100% 무손실 엔진 (신전법 '불노자위' 데이터 정규화 완료)
console.log("[시스템 분석] tactic_dogam.js 고속 해시 맵 렌더러 기동 (계승 전법 82종 클린 렌더링)");
...
var tacticDogamData = [
    ...
    { id: 't_gyeokan', name: '격안관화', type: '지휘 (100%)', target: '아군 전체', desc: '매 턴 시작 시 65%의 확률로 아군 전체의 병력을 회복(치료율 40%, 모략의 영향 받음)하고 받는 피해를 20% 감소(모략의 영향 받음)시키며, 아군 후열 대상 1명의 연격률을 40% 상승(모략의 영향 받음)시킵니다. 해당 효과는 턴 종료 시까지 지속됩니다.' },
    ...
    { id: 't_yubi', name: '유비무환', type: '지휘 (100%)', target: '아군 2팀', desc: '매 턴 시작 시 70% 확률로 아군 2명의 병력을 크게 회복시키고(치료율 150%, 모략의 영향을 받음), 전투 종료 시까지 아군이 받는 피해를 15% 감소시킵니다.' },
    ...
    { id: 't_pojeon', name: '포전인옥', type: '패시브 (100%)', target: '자신, 아군 2팀', desc: '무장의 고유 능동 전법 발동 확률이 5% 상승하고(모략이 무용보다 높을 경우 계수 5% 상승), 모략이 10% 상승합니다(해제 불가). 능동 전법 발동 후, 자신 및 아군 대상 1명의 병력을 회복시킵니다(치료율 155%, 모략의 영향 받음. 매 턴 최대 1회 발동).' },
    
    // 🚨 신전법 불노자위 데이터 추가
    { id: 't_bulno', name: '불노자위', type: '지휘 (100%)', target: '아군 전체', desc: '처음 3턴 동안 매 턴 시작 시, 아군 전체가 받는 무용 피해 및 모략 피해를 20% 감소시키며(통솔의 영향 받음. 대상이 전열일 경우 계수가 20% 상승하지만 가하는 피해가 24% 감소), 턴 종료 시까지 지속됩니다. 4턴 및 5턴 시작 시, 아군 전체의 병력을 회복시키고(치료율 140%, 통솔의 영향 받음), 후열이 가하는 피해를 12% 상승시킵니다(턴 종료 시까지 지속).' },
    ...
    { id: 't_hujeok', name: '후적박발', type: '패시브 (100%)', target: '자신, 적군 1팀', desc: '적군의 제어기(침묵/겁전)를 무시하고 턴 종료 시 무작위 적군 1명에게 강력 모략 피해를 확정적으로 가합니다. 전투 지속될수록 피해 계수 상승.' }, 
...
var TACTIC_MASTER_DESC = {
    ...
    "경달권변": { role: "능동 (65%)", target: '적군 단체', desc: '적군 단체에 혼란 효과를 부여하고 강력 모략 피해를 가합니다.' },
    ...
    "포전인옥": { role: "패시브 (100%)", target: "자신, 아군 2팀", desc: "고유 능동 전법 발동 확률을 최대 10% 상승시키고 모략을 10% 영구 증가시킵니다. 능동 전법 시전 후 자신과 아군 1명을 크게 치료합니다." },
    
    // 🚨 불노자위 데이터 추가
    "불노자위": { role: "지휘 (100%)", target: "아군 전체", desc: "1~3턴간 아군 전체 피감(전열은 피감 대폭 상승 및 가하는 피해 감소). 4~5턴간 전체 폭힐 및 후열 데미지 증폭. 방어형 덱의 초반 생존과 후반 스윕을 완벽히 지원하는 0티어 전법." }
};
