// 삼국지 전략판 65개 실제 전법 데이터 베이스 (가나다 사전식 순서 정렬 완료)
const tacticDogamData = [
    ...
    { id: 't_geumnang', name: '금낭묘계', type: '지휘 (100%)', target: '아군 전체', desc: '첫 3턴 내 매 턴 시작 시, 아군 전체가 받는 피해를 각각 30% > 20% > 10%만큼 감소시키고, 턴 종료 시 아군 중 병력이 가장 낮은 대상의 병력을 회복함(치료율 55%, 모략의 영향).' },
    ...
    { id: 't_yangui', name: '양의화생', type: '능동 (50%)', target: '자신, 적군 2팀', desc: '자신에게 2턴 동안 다모를 부여합니다. 적군 대상 2명에게 160%의 모략 피해를 입히며, 대상의 무용이 모략보다 높을 경우 피해 계수가 20% 상승합니다. 반대로 대상의 모략이 더 높을 경우, [양의화생]으로 입힌 피해의 30%만큼 자신의 병력을 회복합니다.' },
    ...
    { id: 't_yongwang', name: '용왕직전', type: '추격 (50%)', target: '자신, 적군 2팀', desc: '일반 공격 후 자신에게 용맹 상태를 2턴간 부여합니다. 그 후 적 목표 2명에게 115%의 무용 피해를 입히며, 자신의 무용이 목표보다 높을 경우 피해 계수가 27% 증가합니다.' },
    ...
    { id: 't_jilpung', name: '질풍노도', type: '능동 (70%)', target: '자신, 적군 2팀', desc: '자신의 파갑 15% 상승(무용의 영향 받음), 2턴 동안 지속됩니다. 동시에 적군 대상 2명에게 110%의 무용 피해를 입히며, 40%의 확률(무용의 영향 받음)로 무용이 가장 낮은 적군 대상에게 110%의 무용 피해를 추가로 입힙니다.' },
    ...
    { id: 't_taecheong', name: '태청단경', type: '패시브 (50%)', target: '아군 2팀', desc: '피해를 입은 후, 아군 2명의 병력을 회복하고(치료율 45%, 모략에 영향받음), 50%의 확률로 디버프 1개를 해제합니다. 매 턴 최대 5회 발동합니다.' },
...
function renderTacticDogam() {
    const container = document.getElementById('tactic-list');
    if (!container) return;
    container.innerHTML = '';

    tacticDogamData.forEach(data => {
        const card = document.createElement('div');
        // 전법 도감 공통 스킨 공유를 위한 도감 카드 바인딩 (자체 퍼플 테두리 적용)
        card.className = 'dogam-card';
        card.style.borderTop = '6px solid #7b2cb0'; // 전법 전용 보라색 탑 라인
        ...
        card.innerHTML = `
            ...
            <div class="card-body">
                <div class="profile-info">
                    <div><span class="label-orange">역할:</span>${data.type}</div>
                    <div><span class="label-orange">대상:</span>${data.target}</div>
                </div>
            </div>
            <div class="skill-box">
                <div class="skill-name">전법 효과 분석</div>
                <div>${data.desc}</div>
            </div>
        `;
