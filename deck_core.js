...
// ==========================================================================
// [이관 완료] EQUIP_ATTR_DESCRIPTIONS 정적 사전은 guide.js로 이관되어 코드 용량이 약 35줄 압축되었습니다.

// [경량화] 공백 및 특수문자 무시 고속 정규화 헬퍼
const cStr = s => s?.toString().trim().replace(/\s+/g, '') || "";

// [고도화] 도감 미연동 시 즉시 응답하는 55명 고유 전법 + S/A급 주력 전법 통합 마스터 사전
const TACTIC_MASTER_DESC = {
    // 1. 주요 무장 고유 전법 (장수 도감 소속 데이터 100% 커버)
    ...
    "청백충근": { role: "능동 (60%)", target: '아군 주장', desc: '아군 주장의 모략 크리티컬 확률을 증폭하고 피해를 대신 숄더링합니다.' },
...
// ==========================================================================
// LAYER 4: UI 파이프라인 및 모달 컨트롤 (3단 폴백 도감 연동 종결 팝업 엔진)
// ==========================================================================
...
// [고도화] DOM 스크래핑 철거 -> 3단 다중 폴백(장수 도감 -> 전법 도감 -> 내장 사전) 동기식 연계
window.showTacticPopup = function(e, tacticName) {
    ...
    // 2차: 장수 도감 API 조회 (무장 고유 전법인 경우 대응)
    ...
    // 3차: 독립 실행 또는 오프라인 환경을 위한 내장 마스터 사전 조회
    if (!tData) tData = TACTIC_MASTER_DESC[cleanName] || TACTIC_MASTER_DESC[tacticName];
    ...
    if (tData) {
        ...
        // 4차 최후의 수단: 내부 연산 가중치 테이블을 역파싱하여 동적 설명 표출
