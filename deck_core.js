// [시스템 분석] deck_core.js - 통합 연산 기반 Perfect Recommendation (완벽 추천/검증) 엔진 탑재
...
const internalTacticStatMap = {
    ...
    "군민위로": { damageTakenRed: 12 }, "잠피기봉": { damageTakenRed: 15 }, "초선차전": { healGiven: 10 },
    "봉시진": { damageDealtInc: 15 }, "팔문금쇄진": { damageTakenRed: 8 }
};
...
// [로직 추가] 계산된 스탯 총합을 분석하여 완벽한 세팅을 유도하는 'Perfect Recommendation' 함수
function evaluateDeckPerfection(deck, metaId) {
    ...
    const isLongBattle = (metaId === 'wei_sima_shield' || metaId === 'shu_evasion_bangwon');
    const isBurstDmg = (metaId === 'wei_flawless_assassin' || metaId === 'qun_cavalry' || metaId === 'qun_whitehorse_bow');

    if (totalDmgRed < 25) {
        recLogs.push(`⚠️ <strong>[생존력 취약]</strong> 부대 총합 피해 감소(${totalDmgRed.toFixed(1)}%)가 25% 미만입니다. '잠피기봉', '군민위로', '팔문금쇄진' 등의 채용을 강력히 추천합니다.`);
    }
    
    if (isBurstDmg && totalDmgInc < 30) {
        recLogs.push(`⚠️ <strong>[결정력 부족]</strong> 암살/돌파 덱 메타임에도 피해 증가 스탯(${totalDmgInc.toFixed(1)}%)이 낮습니다. '봉시진' 등의 공격적 버프를 기용하십시오.`);
    }

    if (isLongBattle && totalHeal < 15) {
        recLogs.push(`⚠️ <strong>[유지력 경고]</strong> 장기전 메타 덱에서 회복/치유 보정(${totalHeal.toFixed(1)}%)이 부족합니다. '초선차전', '관골요상' 등 유지력 전법을 보충하십시오.`);
    }
    ...
    if (recLogs.length === 0 && allTacticsFilled) {
        return `<div class="feedback-item success" style="border: 1px solid #4ade80; background: rgba(74, 222, 128, 0.1); padding: 8px; margin-top: 10px;">✨ <strong>[최종 검증 완료: Perfect Synergy]</strong> 공방 밸런스와 필수 스탯 임계치를 완벽하게 달성한 종결 세팅입니다. 실전에 즉시 투입하십시오.</div>`;
    }
