...
// [규격 오류 교정] 투구 1차(피해 가함 삭제 -> 무용 피해 가함), 투구 2차(무용 피해 가함 삭제 -> 창병 피해 가함) 반영
const EQ_PRESETS = {
    PHYS_CARRY: ["호분관","무용 피해 가함","창병 피해 가함","명광갑","무용 피해 가함","창병 배반, 공심 상승","치룡패","무용 피해 가함","창병 배반, 공심 상승"],
...
// [구문 오류 100% 교정 완료] "위연" 객체 끝 쉼표 복구 및 공식 규칙 정규화
...
// [경량화] 파편화된 진형 효과 및 배치 위치 정보를 단일 FORMATIONS 마스터 객체로 압축
const FORMATIONS = {
...
// [경량화] 무거운 JSON 직렬화 깊은 복사 대신 고속 얕은 복사 매핑 적용
...
function getOfficerEquipment(officerName, deckUnitType = "") {
    ...
    if (rawEq) {
        // [경량화] 무거운 JSON 직렬화 대신 객체 구조 분해 할당으로 고속 복사
