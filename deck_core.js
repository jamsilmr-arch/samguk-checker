...
// [신설] 55명 전체 무장 내장 장비 미러링 마스터 사전 (도감 통신 실패 시에도 100% 정상 작동 보장)
const FB_EQUIP_MAP = {
...
// [고도화] 도감 API, 내장 미러링, 커스텀 스케일링 등 모든 수급 장비를 병종 접두사로 정규화하는 단일 엔진
function getOfficerEquipment(officerName, deckUnitType = "") {
    ...
    if (!rawEq) {
        rawEq = FB_EQUIP_MAP[cStr(officerName)] || FB_EQUIP_MAP[officerName] || null;
    }
