...
// [신설] 도감 미연동 시에도 정확한 수치 설명을 100% 보장하는 내장 전법 마스터 딕셔너리
...
// [고도화] 도감 API, 내장 명세 사전, 고유 전법 탐색까지 완벽 결선된 4단 폴백 팝업 뷰어
window.showTacticPopup = function(e, tacticName) {
    ...
    // 1차: 전법 도감 API 조회 (일반 전법)
    let tData = window.getTacticDataFromDogam ? window.getTacticDataFromDogam(tacticName) : null;
    
    // 2차: 내장 전법 마스터 사전 조회 (도감 미연동 시 100% 공식 명세 표출)
    if (!tData && FB_TACTIC_DESC_MAP[cleanName]) {
        tData = FB_TACTIC_DESC_MAP[cleanName];
    }

    // 3차: 장수 도감 고유 전법 탐색 (도감 브릿지 연동)
