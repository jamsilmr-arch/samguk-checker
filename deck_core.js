console.log("[시스템 분석] deck_core.js 논리적 다이어트 및 메모리 최적화 엔진 기동");
...
// [경량화]: 수동 하드코딩 배열 삭제 후 딕셔너리 Key 기반 동적 파싱으로 1차 압축
...
// [경량화]: 매 렌더링 연산 시마다 생성되던 메모리 포식 객체를 전역(Global)으로 분리하여 가비지 컬렉션 부하 원천 차단
...
function getOfficerEquipment(officerName) {
    if (typeof window.getEquipmentRecommendationFromGuide === 'function') return window.getEquipmentRecommendationFromGuide(officerName);
    return { helmet: { name: "진현관", attr1: "피해 감소", attr2: "피해 가함" }, armor: { name: "명재복", attr1: "피해 감소", attr2: "모략 피해 가함" }, accessory: { name: "박산로", attr1: "치유 효과 상승", attr2: "방패병 피해 감소" } };
}
        ...
        if (metaIdx !== -1) {
            ...
            if (Array.isArray(off.chosenTactics)) {
                ...
                off.chosenTactics.forEach((tac, tIdx) => {
                    ...
                    if (!tacticDataMap[cleanTac]?.isOwned) fb.logs.push({ type: 'warning', text: `자원 부족: [${hName}]의 ${tIdx + 2}번 슬롯 전법 <strong>[${rawTac}]</strong>이 미보유 상태입니다.` });
                });
            }
        } else {
        ...
        dynamicPresetDecks.forEach((deck) => {
            ...
            deckCard.innerHTML = `
                <div class="deck-title" style="display:flex; justify-content:space-between; align-items:center; width:100%;">
                    <div><span contenteditable="true" onblur="saveEditedText(${deck.originIdx}, 'title', this)" style="outline:none;">${deck.title}</span><span style="color:#ff9f43; font-size:13px; margin-left:12px; font-weight:bold; user-select:none;">[추천도: ${currentComputedScore}점]</span></div>
